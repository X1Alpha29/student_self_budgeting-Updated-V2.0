<div class="container mt-4">
    <div class="row">
        ...
        <div class="col-md-4 mb-4">
            <div class="card shadow clickable-card" data-url="{{ route('expenses.index') }}">
                <div class="card-body">
        ...
        <div class="col-md-4 mb-4">
            <div class="card shadow clickable-card" data-url="{{ route('debits.index') }}">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
        ...
        <div class="col-md-4 mb-4">
            <div class="card shadow clickable-card" data-url="{{ route('debts.index') }}">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                ...
                <div class="col-md-8 mb-4">
                    <div class="card shadow" style="background-color: white;">
                        <div class="card-body" style="position: relative; height: 400px;">
                            <canvas id="incomeOverTimeChart" height="400"></canvas>
                        </div>
                        <div class="card-footer">
                            Current Month: {{ \Carbon\Carbon::now()->format('F Y') }}
                        </div>
...
<script>
        ...
        function fetchUpcomingDirectDebitsSummary() {
            fetch("{{ route('upcoming-direct-debits') }}")
                .then(response => response.json())
                .then(data => {
                ...
                .then(data => {
                    ...
                    if (data.length > 0) {
                        ...
                        data.forEach(debt => {
                            ...
                            const dueDateString = !isNaN(dueDate) ? dueDate.toLocaleDateString('en-GB', {
                                year: 'numeric', month: 'long', day: 'numeric'
                            }) : "Invalid Date";

                            // Append remaining debt amount to the information string
                            nextDebtInfo += `${debt.name} due on ${dueDateString}, £${debt.remaining.toFixed(2)} remaining\n`;
                        });

                        // Use innerHTML to retain formatting such as line breaks in the browser display
                        document.getElementById('nextDebtsWithinMonth').innerHTML = nextDebtInfo.replace(/\n/g, '<br>');
                    } else {
    ...
    function fetchMonthlyIncomeOverTime() {
    fetch("{{ route('monthly-income-over-time') }}")
        .then(response => response.json())
        .then(data => {
            renderMonthlyIncomeOverTimeChart(data);
        })
        .catch(error => console.error('Error fetching monthly income over time:', error));
    }

    function renderMonthlyIncomeOverTimeChart(data) {
    var ctx = document.getElementById('incomeOverTimeChart').getContext('2d');
    var incomeOverTimeChart = new Chart(ctx, {
        type: 'line',
        data: {
            labels: data.labels,
            datasets: [{
